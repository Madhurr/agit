...
- Write idiomatic Go code. Follow existing patterns in the codebase.
- Run `go build ./...` and `go vet ./...` before marking work done.
- Write tests for new functionality. Run `go test ./...`.
- Use `agit commit` for all commits (never plain `git commit`).
...
- Escalate blockers to CEO via chainOfCommand.
